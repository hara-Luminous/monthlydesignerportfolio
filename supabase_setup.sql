...
-- 既存プロジェクトにも安全に追加できる月別分類カラム
alter table public.portfolio_works
  add column if not exists work_month smallint;

do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'portfolio_works_work_month_check'
  ) then
    alter table public.portfolio_works
      add constraint portfolio_works_work_month_check check (work_month between 1 and 12);
  end if;
end $$;

-- 既存作品は created_at の月を初期値として設定
update public.portfolio_works
set work_month = extract(month from created_at)::smallint
where work_month is null;
...
insert into public.portfolio_works (id,title,category,work_month,tags,note,published,image_url,storage_path,sort_order)
values
('00000000-0000-4000-8000-000000000001','Girly Recruitment Visual','かわいい',9,array['CUTE','PINK','RECRUITING'],'',true,'assets/work-01.jpg',null,0),
('00000000-0000-4000-8000-000000000002','Luxury Recruitment Visual','高級感',9,array['LUXURY','GOLD','RECRUITING'],'',true,'assets/work-02.jpg',null,1),
('00000000-0000-4000-8000-000000000003','Neon Recruitment Visual','ネオン',9,array['NEON','BLUE','RECRUITING'],'',true,'assets/work-03.jpg',null,2)
